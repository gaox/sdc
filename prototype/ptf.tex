...
\frame
{
	...
	\begin{itemize}
		...
		\item An $O(n^{2.5})$ time complexity can be achieved by finding many augmenting paths at one time, see Micali and Vazirani[80].
	\end{itemize}
...
\frame
{
	\frametitle{Summary}
	\begin{itemize}
		\item We have found the dual problem to maximum matching on general graphs!
		\item Although maximum matching is an integer programming, which is inherently intractable, Edmonds proved in another paper that the optimal value of this integer programming is equal to the optimal value of the corresponding linear programming problem, if we relax the integral constraints. But it is beyond the scope of this talk.
	\end{itemize}
}
